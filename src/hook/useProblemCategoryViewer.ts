...
export default function useCategoryViewer(
  initialState: ProblemCategoryState = "hide",
) {
  const [categoryState, setCategoryState] =
    useState<ProblemCategoryState>(initialState);
  const [tooltipContent, setTootipContent] = useState(
    initialState === "hide"
      ? "유형 보기"
      : initialState === "view"
        ? "유형 숨기기"
