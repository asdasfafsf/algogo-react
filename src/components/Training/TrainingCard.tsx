    ...
    >
      <div className={`flex items-center gap-4 ${!isActive ? 'opacity-60' : ''}`}>
        ...
        <div className={`
          ...
          bg-linear-to-br ${config.gradient} rounded-xl
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`h-0.5 bg-linear-to-r ${config.accent} rounded-full mt-3 transition-all duration-300 ${
      ...
      {/* 호버 오버레이 */}
      {isActive && (
        <div className={`absolute inset-0 bg-linear-to-br ${config.overlay} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg`} />
      )}

      {/* 준비중 배지 */}
      {status === 'coming-soon' && (
        <div className="absolute flex items-center gap-1 px-3 py-1.5 text-xs bg-linear-to-r from-gray-600 to-gray-700 rounded-full top-4 right-4">
      ...
      {/* 비활성 오버레이 */}
      {status === 'coming-soon' && (
        <div className="absolute inset-0 rounded-lg bg-linear-to-br from-gray-500/5 to-transparent" />
      )}
    </Card>
  );
}
