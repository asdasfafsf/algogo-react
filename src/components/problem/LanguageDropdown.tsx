...
export default function LanguageDropdown() {
  const {open, handler, selectedIndex, languageList, handleUpdate } = useLanguageDropdown();

  return (
    <Dropdown 
      handler={handler}
      open={open}
      className='p-0  border-gray-800 bg-gray-900 rounded-md'
      showArrow={false}>
      <div 
      ...
      <ul className='w-32 p-0 bg-gray-900 rounded-sm'>
        {
          languageList.map((elem, index) => (
            <li
              onClick={(e) => handleUpdate(e as any, index)}
              className='flex items-center w-full gap-1 p-3 rounded-md cursor-pointer hover:bg-gray-600 bg-gray-900'
              >
              <Typography 
              className="text-gray-400"
              weight='semilight'
              variant={'medium'}>{elem}</Typography>
            </li>
          ))
        }
      </ul>
