...
try {
  const todayProblemPage = await server.ssrLoadModule(
    "/src/domain/problems/todayProblemPage.ts",
  );
  ...
  test("오늘의 문제 UI 날짜 범위를 API 조회 계약에 맞춘다", () => {
    const today = new Date(2026, 8, 10, 13, 30);

    assert.equal(todayProblemPage.normalizeTodayProblemDay(null, today), 0);
    assert.equal(todayProblemPage.normalizeTodayProblemDay("-30", today), -30);
    assert.equal(todayProblemPage.normalizeTodayProblemDay("-31", today), 0);
    assert.equal(todayProblemPage.normalizeTodayProblemDay("1", today), 0);
    assert.equal(todayProblemPage.normalizeTodayProblemDay("abc", today), 0);
    assert.equal(
      todayProblemPage.dayOffsetFromDateInput("2026-09-09", today),
      -1,
    );
    assert.equal(
      todayProblemPage.dayOffsetFromDateInput("2026-02-31", today),
      null,
    );
  });
  ...
  test("문제 도메인은 UI와 I/O 라이브러리에 의존하지 않는다", async () => {
    ...
    for (const sourceFile of sourceFiles) {
      const source = await readFile(
        resolve(domainDirectory, sourceFile),
        "utf8",
      );
